...
const EditableContent = () => {
  ...
  return (
    ...
    <div>
  <div class="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    <div class="p-6">
      ...
      <div id="terms-sections" class="space-y-4">
  ...
  {/* <script>
    ...
    sections.forEach(section => {
      ...
      header.className = 'flex justify-between items-center cursor-pointer group';
      header.innerHTML = `
        <h3 class="flex items-center text-lg font-semibold text-gray-700">
          <span class="flex items-center justify-center w-7 h-7 bg-blue-500 text-white rounded-full mr-3 text-sm">
            ${section.id}
          </span>
          ${section.title}
        </h3>
        <button class="text-gray-400 hover:text-blue-500 transition-transform duration-200 transform" id="icon-${section.id}">
      ...
      contentDiv.className = `mt-3 text-gray-600 hidden ${section.isImportant ? 'bg-red-50 p-4 rounded-md border-l-4 border-red-400' : ''}`;
      ...
      header.addEventListener('click', () => {
        ...
        const icon = document.getElementById(`icon-${section.id}`).firstElementChild;
