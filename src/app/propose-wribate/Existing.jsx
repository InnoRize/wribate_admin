...
export default function ExistingProposeWribate() {
  ...
  const fetchWribates = async (reset = false) => {
    try {
      ...
      const res = await axios.get(process.env.NEXT_PUBLIC_APP_BASE_URL + '/propose/' + selectedCategory + '?lastId=' + id+"&user_id="+user?._id);
      const data = res.data;
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (data.status === 1) {
        ...
        Toast("Proposal deleted successfully","success");
      }
    } catch (err) {
      console.error(err);
